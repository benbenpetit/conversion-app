...
//  - une liste de nombres pour convertir mile->yard, yard->foot... (utile seulement imperial->si)
...
export function converter(resultList, inputValue, convertFactorList) {
  if (resultList[0].kind == 'si') {
    var formatValue = inputValue.toString().split('').map(Number).reverse(); //On crée un tableau avec les valeurs en unités SI (reverse seulement utile pour l'affichage)
  } else {
...
//Recoit un nombre en Mile et une liste de nombre pour faire la conversion
...
function formatImperialToSi(number, convertFactorList) {
  ...
  convertFactorList.map((convertFactor, i) => {
    ...
    number = (number - Math.trunc(number)) * convertFactor; //On convertit le quotient avec l'unité inférieur (mile->yard...)
  });
  list[convertFactorList.length] = number.toFixed(2); //Sans oublier le reste du dernier élément (on garde 2 décimales pour voir un changement)
  return list;
}
